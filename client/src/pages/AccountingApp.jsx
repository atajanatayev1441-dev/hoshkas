...
function AIAnalyst() {
  ...
  // Быстрые вопросы
  const QUICK = [
    'Проанализируй выручку за последние 30 дней и выяви тренды',
    'Какие блюда самые прибыльные и какие стоит убрать из меню?',
    'Есть ли подозрительные расходы или аномалии?',
    'Как оптимизировать расходы на следующий месяц?',
    'Кто из персонала приносит больше всего выручки?',
    'На каких позициях меню самая низкая маржа?',
    'Составь краткий финансовый отчёт за сегодня',
    'Какие товары на складе скоро закончатся?',
  ]
  ...
  async function loadContext() {
    ...
    try {
      ...
      const [summary, expenses, recentOrders, stock, rejected, staff, shifts, margin] = await Promise.all([
        fetch(`/api/accounting/full-summary?from=${month_ago}&to=${today}`).then(r=>r.ok?r.json():null).catch(()=>null),
        fetch(`/api/accounting/expenses?from=${month_ago}&to=${today}`).then(r=>r.ok?r.json():[]).catch(()=>[]),
        fetch(`/api/orders/recent?limit=100`).then(r=>r.json()).catch(()=>[]),
        fetch(`/api/warehouse/products`).then(r=>r.ok?r.json():[]).catch(()=>[]),
        fetch(`/api/atlant/rejected`).then(r=>r.json()).catch(()=>[]),
        fetch(`/api/atlant/staff`).then(r=>r.json()).catch(()=>[]),
        fetch(`/api/shifts?limit=10`).then(r=>r.json()).catch(()=>[]),
        fetch(`/api/accounting/margin?from=${month_ago}&to=${today}`).then(r=>r.ok?r.json():null).catch(()=>null),
      ])

      // Считаем базовую статистику из заказов
      const paidOrders = (Array.isArray(recentOrders)?recentOrders:[]).filter(o=>o.status==='PAID')
      const totalRevenue = paidOrders.reduce((s,o)=>s+o.total,0)
      const cashRevenue = paidOrders.filter(o=>o.paymentType==='CASH').reduce((s,o)=>s+o.total,0)
      const cardRevenue = paidOrders.filter(o=>o.paymentType==='CARD').reduce((s,o)=>s+o.total,0)
      ...
      const topItems = Object.values(itemMap).sort((a,b)=>b.total-a.total).slice(0,15)
      ...
      // Склад — критически мало
      const lowStock = (Array.isArray(stock)?stock:[]).filter(p=>(p.quantity||0)<(p.minQuantity||5))

      const ctx = {
        period: `${month_ago} — ${today}`,
        revenue: { total: totalRevenue, cash: cashRevenue, card: cardRevenue, orders: paidOrders.length, avgCheck },
        expenses: { total: totalExpenses, byCategory: expByCategory },
        profit: totalRevenue - totalExpenses,
        topItems,
        lowStock: lowStock.slice(0,10).map(p=>({name:p.name,qty:p.quantity,unit:p.unit})),
        rejectedLoss: (Array.isArray(rejected)?rejected:[]).reduce((s,r)=>s+r.total,0),
        rejectedCount: (Array.isArray(rejected)?rejected:[]).length,
        staff: Array.isArray(staff)?staff:[],
        shiftsCount: (Array.isArray(shifts)?shifts:[]).length,
        margin: margin ? { totalMargin: margin.totalMargin, marginPct: margin.marginPct } : null,
        worstItems: margin ? (margin.items||[]).filter(i=>i.marginPct<30).slice(0,5) : [],
      }
      setContext(ctx)
      return ctx
  ...
  async function sendMessage(text) {
    ...
    try {
      ...
      const systemPrompt = `Ты опытный финансовый аналитик и бухгалтер ресторана HOS LOUNGE. 
Ты помогаешь бухгалтеру анализировать данные, выявлять проблемы и принимать решения.
Отвечай конкретно, по делу, на русском языке. Используй цифры из данных.
Если видишь проблему — назови её прямо. Давай практические рекомендации.

ТЕКУЩИЕ ДАННЫЕ РЕСТОРАНА (период ${ctx?.period || 'последние 30 дней'}):

ВЫРУЧКА:
- Всего: ${fmt(ctx?.revenue?.total)} TMT за ${ctx?.revenue?.orders} чеков
- Наличными: ${fmt(ctx?.revenue?.cash)} TMT
- Картой: ${fmt(ctx?.revenue?.card)} TMT  
- Средний чек: ${fmt(ctx?.revenue?.avgCheck)} TMT

РАСХОДЫ:
- Всего: ${fmt(ctx?.expenses?.total)} TMT
- По категориям: ${JSON.stringify(ctx?.expenses?.byCategory)}

ПРИБЫЛЬ: ${fmt(ctx?.profit)} TMT (${ctx?.revenue?.total ? ((ctx.profit/ctx.revenue.total)*100).toFixed(1) : 0}% маржа)

ТОП БЛЮД ПО ВЫРУЧКЕ:
${(ctx?.topItems||[]).map((i,n)=>`${n+1}. ${i.name}: ${i.qty} шт — ${fmt(i.total)} TMT`).join('\n')}

МАРЖИНАЛЬНОСТЬ: ${ctx?.margin ? `${fmt(ctx.margin.marginPct)}% валовая маржа` : 'нет данных'}
НИЗКОМАРЖИНАЛЬНЫЕ БЛЮДА (<30%): ${(ctx?.worstItems||[]).map(i=>`${i.name} (${fmt(i.marginPct)}%)`).join(', ') || 'нет'}

ОТКАЗНЫЕ ЧЕКИ: ${ctx?.rejectedCount} штук, потери ${fmt(ctx?.rejectedLoss)} TMT

ПЕРСОНАЛ:
${(ctx?.staff||[]).map(s=>`- ${s.name}: ${fmt(s.total_with_service)} TMT`).join('\n')}

СКЛАД (заканчивается):
${(ctx?.lowStock||[]).map(p=>`- ${p.name}: ${p.qty} ${p.unit}`).join('\n') || 'нет критических остатков'}

Смен проведено: ${ctx?.shiftsCount}`
  ...
  const fmt2 = n => Number(n||0).toLocaleString('ru-RU',{maximumFractionDigits:0})

  return (
    <div style={{display:'flex',height:'calc(100vh - 116px)',overflow:'hidden'}}>

      {/* Левая панель — контекст данных */}
      <div style={{width:260,flexShrink:0,borderRight:'1px solid #e8e8e8',overflow:'auto',background:'#fafafa',padding:16}}>
        <div style={{fontSize:13,fontWeight:700,color:'#1a1a2e',marginBottom:12,display:'flex',alignItems:'center',justifyContent:'space-between'}}>
          Данные
          <button onClick={loadContext} disabled={contextLoading} style={{background:'none',border:'1px solid #e0e0e0',borderRadius:6,padding:'3px 8px',fontSize:11,cursor:'pointer',color:'#888',fontFamily:'inherit'}}>
            {contextLoading ? '...' : '↻'}
          </button>
        </div>
        ...
        {context && (
          <>
            <div style={{marginBottom:12}}>
              <div style={{fontSize:10,fontWeight:700,color:'#aaa',textTransform:'uppercase',letterSpacing:0.5,marginBottom:6}}>Выручка (30 дней)</div>
              <div style={{fontSize:18,fontWeight:800,color:'#27ae60'}}>{fmt2(context.revenue.total)} TMT</div>
              <div style={{fontSize:11,color:'#aaa',marginTop:2}}>{context.revenue.orders} чеков · ср. {fmt2(context.revenue.avgCheck)} TMT</div>
            </div>

            <div style={{marginBottom:12}}>
              <div style={{fontSize:10,fontWeight:700,color:'#aaa',textTransform:'uppercase',letterSpacing:0.5,marginBottom:6}}>Расходы</div>
              <div style={{fontSize:16,fontWeight:700,color:'#e74c3c'}}>{fmt2(context.expenses.total)} TMT</div>
            </div>

            <div style={{marginBottom:12}}>
              <div style={{fontSize:10,fontWeight:700,color:'#aaa',textTransform:'uppercase',letterSpacing:0.5,marginBottom:6}}>Прибыль</div>
              <div style={{fontSize:16,fontWeight:700,color:context.profit>=0?'#2980b9':'#e74c3c'}}>{fmt2(context.profit)} TMT</div>
            </div>

            {context.lowStock.length>0 && (
              <div style={{marginBottom:12,background:'#fff8ec',borderRadius:8,padding:'8px 10px',border:'1px solid #f9ca8b'}}>
                <div style={{fontSize:10,fontWeight:700,color:'#e67e22',textTransform:'uppercase',marginBottom:5}}>⚠ Мало на складе</div>
                {context.lowStock.slice(0,4).map(p=>(
                  <div key={p.name} style={{fontSize:11,color:'#555',marginBottom:2}}>{p.name}: {p.qty} {p.unit}</div>
                ))}
              </div>
            )}

            {context.rejectedLoss>0 && (
              <div style={{marginBottom:12,background:'#fdf2f2',borderRadius:8,padding:'8px 10px',border:'1px solid #f5c6c6'}}>
                <div style={{fontSize:10,fontWeight:700,color:'#e74c3c',textTransform:'uppercase',marginBottom:4}}>Потери (отказные)</div>
                <div style={{fontSize:13,fontWeight:700,color:'#e74c3c'}}>{fmt2(context.rejectedLoss)} TMT</div>
                <div style={{fontSize:11,color:'#aaa'}}>{context.rejectedCount} позиций</div>
              </div>
            )}

            <div style={{fontSize:10,fontWeight:700,color:'#aaa',textTransform:'uppercase',letterSpacing:0.5,marginBottom:6}}>Топ блюд</div>
            {context.topItems.slice(0,6).map((i,idx)=>(
              <div key={i.name} style={{display:'flex',justifyContent:'space-between',fontSize:11,marginBottom:4,color:'#555'}}>
                <span style={{overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap',maxWidth:140}}>{i.name}</span>
                <span style={{color:'#27ae60',fontWeight:600,flexShrink:0,marginLeft:4}}>{fmt2(i.total)}</span>
              </div>
            ))}
          </>
        )}
        {contextLoading && <div style={{textAlign:'center',padding:20,color:'#aaa',fontSize:12}}>Загрузка данных...</div>}
      </div>
      ...
      <div style={{flex:1,display:'flex',flexDirection:'column',overflow:'hidden'}}>
        ...
        <div style={{flex:1,overflow:'auto',padding:'20px 24px'}}>
          ...
          {messages.length===0 && (
            <div style={{maxWidth:640,margin:'0 auto'}}>
              <div style={{textAlign:'center',marginBottom:28}}>
                <div style={{width:56,height:56,borderRadius:18,background:'linear-gradient(135deg,#1a1a2e,#2980b9)',display:'flex',alignItems:'center',justifyContent:'center',margin:'0 auto 12px',fontSize:24}}>🤖</div>
                <div style={{fontSize:20,fontWeight:800,color:'#1a1a2e',marginBottom:6}}>AI-Аналитик HOS LOUNGE</div>
                <div style={{fontSize:13,color:'#888',lineHeight:1.5}}>
                  Я анализирую ваши финансовые данные в реальном времени.<br/>
                  Задайте вопрос или выберите готовый анализ.
                </div>
              </div>
              ...
              <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:8}}>
                {QUICK.map(q=>(
                  <button key={q} onClick={()=>sendMessage(q)}
                    style={{background:'#fff',border:'1.5px solid #e8e8e8',borderRadius:10,padding:'10px 12px',fontSize:12,color:'#444',cursor:'pointer',textAlign:'left',fontFamily:'inherit',lineHeight:1.4,transition:'all .15s'}}
          ...
          {messages.map((m, i) => (
            <div key={i} style={{display:'flex',gap:10,marginBottom:16,justifyContent:m.role==='user'?'flex-end':'flex-start'}}>
              {m.role==='assistant' && (
                <div style={{width:32,height:32,borderRadius:10,background:'linear-gradient(135deg,#1a1a2e,#2980b9)',display:'flex',alignItems:'center',justifyContent:'center',flexShrink:0,fontSize:14}}>🤖</div>
              )}
              <div style={{
                maxWidth:'75%',
                background: m.role==='user' ? 'linear-gradient(135deg,#1a1a2e,#2c3e6b)' : '#fff',
              ...
              {m.role==='user' && (
                <div style={{width:32,height:32,borderRadius:10,background:'#c9a96e',display:'flex',alignItems:'center',justifyContent:'center',flexShrink:0,fontSize:14,color:'#fff',fontWeight:700}}>Б</div>
              )}
            </div>
          ))}
          ...
          {loading && (
            <div style={{display:'flex',gap:10,marginBottom:16}}>
              <div style={{width:32,height:32,borderRadius:10,background:'linear-gradient(135deg,#1a1a2e,#2980b9)',display:'flex',alignItems:'center',justifyContent:'center',fontSize:14}}>🤖</div>
              <div style={{background:'#fff',borderRadius:'16px 16px 16px 4px',padding:'12px 16px',border:'1px solid #e8e8e8',boxShadow:'0 2px 8px rgba(0,0,0,0.07)'}}>
                <div style={{display:'flex',gap:4,alignItems:'center'}}>
                  {[0,1,2].map(i=>(
                    <div key={i} style={{width:6,height:6,borderRadius:'50%',background:'#c9a96e',animation:'pulse 1.2s infinite',animationDelay:`${i*0.2}s`}}/>
                  ))}
                </div>
              </div>
            </div>
        ...
        {/* Поле ввода */}
        <div style={{borderTop:'1px solid #e8e8e8',padding:'12px 20px',background:'#fff',display:'flex',gap:10,alignItems:'flex-end'}}>
          {messages.length>0 && (
            <button onClick={()=>setMessages([])} title="Очистить чат"
              style={{background:'none',border:'1px solid #e8e8e8',borderRadius:8,padding:'8px 10px',cursor:'pointer',color:'#aaa',flexShrink:0,fontSize:12,fontFamily:'inherit'}}>
              ✕
            </button>
          )}
          <textarea
            ...
            placeholder="Задайте вопрос об финансах... (Enter — отправить, Shift+Enter — новая строка)"
            rows={1}
            style={{flex:1,border:'1.5px solid #e8e8e8',borderRadius:10,padding:'10px 14px',fontSize:13,fontFamily:'inherit',outline:'none',resize:'none',lineHeight:1.5,maxHeight:100,overflowY:'auto'}}
            onFocus={e=>e.target.style.borderColor='#c9a96e'}
            onBlur={e=>e.target.style.borderColor='#e8e8e8'}
          />
          <button onClick={()=>sendMessage(input)} disabled={loading||!input.trim()}
            style={{background:loading||!input.trim()?'#f0f0f0':'linear-gradient(135deg,#1a1a2e,#2c3e6b)',color:loading||!input.trim()?'#aaa':'#fff',border:'none',borderRadius:10,padding:'10px 18px',fontSize:13,fontWeight:700,cursor:loading||!input.trim()?'not-allowed':'pointer',fontFamily:'inherit',flexShrink:0}}>
            Отправить
          </button>
        </div>
      </div>
